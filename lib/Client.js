...
class Client extends discord_js_1.default.Client {
    ...
    async start() {
        ...
        await Load_1.loadCommands(this, this.opt.paths.commands, this.opt.slash);
        await console.log();
        ...
        this.on('interactionCreate', interaction => {
            ...
            if (!command)
                return;
            if (command.permissions.length > 0 && !interaction.member.roles.cache.some(r => command.permissions.includes(r.id)))
                return;
            command.run(this, interaction.channel, interaction.member, interaction);
        });
        this.on("ready", () => {
            if (!this.opt.slash)
                return;
            for (const guildId of this.opt.slash_guilds) {
                ...
                for (const command of this.commands.toJSON().filter(r => r.guilds.includes(guild.id))) {
                    if (!(command instanceof SlashCommand_1.default))
                        continue;
